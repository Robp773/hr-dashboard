...
export default class WeatherEvent extends React.Component {
  ...
  componentDidMount() {
    if ("data" in this.props.eventData[this.props.activeState]) {
      let firstAlert = Object.keys(
        this.props.eventData[this.props.activeState].data
      )[0];
      this.setState({
        tab: firstAlert
      });
  ...
  render() {
    ...
    if (this.props.eventData[this.props.activeState].data) {
      if (
        Object.keys(this.props.eventData[this.props.activeState].data)
          .length === 0
      ) {
        alertsArray = (
          <div
            className="weather-event__no-alerts"
            key="key-no-alerts"
          >{`No Alerts`}</div>
        );
      } else {
        ...
        for (let key in alertsObj) {
          ...
          buttons.push(
            <button
              ...
              className={`weather-event__alerts-h2 ${active}`}
              key={`${key}${this.props.activeState}-header`}
            >
              {key}
              <span className="weather-event__alerts-count">
                {" "}
                ({alertsObj[key].length})
              </span>
            </button>
          );
        }
          ...
          for (let i = 0; i < alertsObj[this.state.tab].length; i++) {
            alertsArray.push(
              <WeatherAlert
                key={i}
                index={i}
                alertsObj={alertsObj[this.state.tab][i]}
              />
            );
          }
        }
    ...
    return (
      <div className="weather-event">
        <div className="weather-event__container">
          ...
          <div className="weather-event__alerts-container">{alertsArray}</div>
        </div>
      </div>
    );
  }
