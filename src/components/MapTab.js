          ...
          .then(res => {
            res.text().then(result => {
              ...
              let riverGaugesTemplate = {
                title: "Gauge Data",
                content:
                  "Location: {LOCATION}, {STATE} <br/> Stage: {STAGE} <br/> Waterbody: {WATERBODY} <br/>  Observation Time: {OBSTIME} <br/> Gauge Link: <a target='#' href={URL}>Visit</a>"
              };

              let riverGaugesLayer = new FeatureLayer({
                url:
                  "https://livefeeds.arcgis.com/arcgis/rest/services/LiveFeeds/StreamGauge/MapServer",
              ...
              riverGaugesLayer.definitionExpression =
                "STAGE='action' OR STAGE='minor' OR STAGE ='moderate' OR STAGE='major'";

              let nwsWatchesTemplate = {
                title: "Alert Data",
                content:
                  "Event: {Event} <br/> Severity: {Severity}<br/> Start: {Start:DateFormat}<br/> End: {End:DateFormat} <br/>  Alert Link: <a target='#' href={Link}>Visit</a>"
              };

              let nwsWatchesLayer = new MapImageLayer({
                visible: true,
                url:
                  "https://livefeeds.arcgis.com/arcgis/rest/services/LiveFeeds/NWS_Watches_Warnings_and_Advisories/MapServer",
                sublayers: [
                  {
              ...
              let wildfireTemplate = {
                title: "Wildfire Data",
                content:
                  "Name: {FIRE_NAME} <br/> Area: {AREA_} Acres <br/> Info Link: <a target='#' href={HOTLINK}>Visit</a>"
              };
              ...
              let precipitationTemplate = {
                ...
                content:
                  "Rainfall: {label} <br/> Start Time: {fromdate:DateFormat} <br/> End Time: {todate:DateFormat}"
              };
              ...
              let snowForecastTemplate = {
                title: "Snow Data",
                content:
                  "Snowfall: {label} in. <br/> Start Time: {fromdate:DateFormat} <br/> End Time: {todate:DateFormat}"
              };

              let snowForecastLayer = new MapImageLayer({
                url:
                  "https://livefeeds.arcgis.com/arcgis/rest/services/LiveFeeds/NDFD_SnowFall/MapServer",
              ...
              let iceTemplate = {
                title: "Ice Data",
                content:
                  "Ice Forecast: {label} in. <br/> Start Time: {fromdate:DateFormat} <br/> End Time: {todate:DateFormat}"
              };

              let iceForecastLayer = new MapImageLayer({
                url:
                  "https://livefeeds.arcgis.com/arcgis/rest/services/LiveFeeds/NDFD_Ice/MapServer",
                visible: disasterCategory === "winterStorm" ? true : false,
                sublayers: [
                  {
                    title: "Cumulative Total",
              ...
              let smokeTemplate = {
                title: "Smoke Data",
                content:
                  "Smoke Desc: {smoke_classdesc} µg/m³ <br/> Start Time: {referencedate:DateFormat} <br/> End Time: {todate:DateFormat}"
              };

              let smokeForecastLayer = new MapImageLayer({
                url:
                  "https://livefeeds.arcgis.com/arcgis/rest/services/LiveFeeds/NDGD_SmokeForecast/MapServer",
                visible: disasterCategory === "fireHeat" ? true : false,
                sublayers: [
                  {
              ...
              let earthquakeTemplate = {
                ...
                content:
                  "Place: {place} <br/> Magnitude: {mag} <br/> Significance: {sig} <br/> Felt: {felt} <br/> Depth: {depth} <br/> Event Time: {eventTime:DateFormat} <br/> Updated: {updated:DateFormat} <br/> Info Link: <a target='#' href={url}>Visit</a> "
              };

              let recentEarthquakesLayer = new MapImageLayer({
                visible: disasterCategory === "earthquake" ? true : false,
                url:
                  "https://livefeeds.arcgis.com/arcgis/rest/services/LiveFeeds/USGS_Seismic_Data/MapServer",
                sublayers: [
                  {
                    title: "Pager Alerts",
                    id: 0,
              ...
              let recentHurricanesLayer = new MapImageLayer({
                url:
                  "https://livefeeds.arcgis.com/arcgis/rest/services/LiveFeeds/Hurricane_Recent/MapServer",
                visible: disasterCategory === "waterRelated" ? true : false
              });
              ...
              let standardLayers = [
                ...
                riverGaugesLayer,
                wildfireLayer,
                recentEarthquakesLayer,
                recentHurricanesLayer
              ];
