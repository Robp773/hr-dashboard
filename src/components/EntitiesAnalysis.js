  ...
  render() {
    ...
    for (let type in this.props.entities) {
      ...
      nameArray.push(
        ...
        >
          {type} ({this.props.entities[type].length})
        </button>
      );
      ...
      for (let i = 0; i < dataArray.length; i++) {
        ...
        activeData.push(
          <tr
            key={`entities-${i}`}
            className={`entities__tr ${i % 2 === 0 ? "td-even" : "td-odd"}`}
          >
            <td className="entities__td entities__td--entity-name">
              {dataArray[i].normalized}
            </td>
            <td className="entities__td">{dataArray[i].count}</td>
            <td className={`entities__td ${colorClass}`}>
              {dataArray[i].confidence ? `${dataArray[i].confidence} %` : "n/a"}
            </td>
          </tr>
        );
      }
    ...
    return (
      <div className="entities">
        <div className="entities__btns-parent">{nameArray}</div>
